...
void irq_init(fptr isr)
{
	...
	REG_ISR_MAIN= (isr) ? isr : (fptr)isr_master;
...
*/
fptr irq_set(enum eIrqIndex irq_id, fptr isr, u32 opts)
{
	u16 ime= REG_IME;
	REG_IME= 0;

	int ii, slot, prio;
	u16 irq_flag= (u16)(BIT(irq_id));
	fptr old_isr;
...
fptr irq_delete(enum eIrqIndex irq_id)
{
	...
	// Disable irq
	const IRQ_SENDER *sender= &__irq_senders[irq_id];
	*(u16*)(REG_BASE+sender->reg_ofs) &= (u16)(~sender->flag);
	REG_IE &= (u16)(~irq_flag);
